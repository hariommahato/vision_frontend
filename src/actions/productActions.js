...
export const getFeaturedProducts = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${url}/api/v1/products/featured`);
...
export const getHotDealProduct = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${url}/api/v1/products/hotdeal`);
...
// Single User product
export const getSingleUserProduct = (id) => async (dispatch) => {
  try {
    dispatch({ type: SINGLE_USER_PRODUCT_REQUEST });

    const { data } = await axios.get(`${url}/api/v1/singleuser/product/${id}`);
